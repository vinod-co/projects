...
<?php
	$sql = "select * from view_service_requests vsr join srresponses sr on vsr.id=sr_id where response_from_id=? order by vsr.id desc";
	...
	if($count == 0){
		?>
		<h3>No service request conversion history :-(</h3>
		<?php
	}
	else{
		?>
		<h3>You have <?php echo  $count; ?> service request conversion<?php echo $count>1?"s":"";?></h3>
		<?php
?>
...
<?php
	...
	<tr>
		...
		<td>
			...
			<button class="submit" data-thread_id="<?php echo $d["thread_id"]; ?>" 
				data-action="view_conversation">Conversation</button>
		</td>
	</tr>
		<?php
	}
