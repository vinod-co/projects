<?php
	...
	$data = R::getAll("select * from users where registration_status = 'declined'");
	$count = count($data);
?>
...
<div id="declined_registrations_container">

	<div id="declined_registrations_header">
		<h1>Declined registrations</h1>
	</div>
...
<?php
	if($count == 0){
		?>
		<h3>No declined registrations :-(</h3>
		<?php
	}
	else{
		?>
		<h3><span id="reg_count"><?php echo  $count; ?></span> declined registration<?php echo $count>1?"s":"";?></h3>
		<?php
	}
?>
...
<?php
	...
	<tr>
		<td>&nbsp;</td>
		<td>
			<a href="view_one_profile_for_approve_decline.php?id=<?php echo $d["id"]; ?>&discriminator=2">
		...
		<td>
			<button class="submit" data-userid="<?php echo $d["id"]; ?>" 
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data.status==true){
						...
						if(regCount==0){
							$("#reg_count").parent().html("No declined registrations :-(");
						}
						else{
