...
<?php
	if($count == 0){
		?>
		<h3>No suspended registrations :-(</h3>
		<?php
	}
	else{
		?>
		<h3><span id="reg_count"><?php echo  $count; ?></span> suspended registration<?php echo $count>1?"s":"";?></h3>
		<?php
	}
?>
...
<?php
	...
	<tr>
		<td>&nbsp;</td>
		<td>
			<a href="view_one_profile_for_approve_decline.php?id=<?php echo $d["id"]; ?>&discriminator=3">
		...
		<td>
			<button class="submit" data-userid="<?php echo $d["id"]; ?>" 
				data-action="approved">Restore</button>
		</td>
	</tr>
		<?php
	}
			...
			$.ajax({
				...
				success: function(data){
					...
					if(data.status==true){
						...
						if(regCount==0){
							$("#reg_count").parent().html("No suspended registrations :-(");
						}
						else{
